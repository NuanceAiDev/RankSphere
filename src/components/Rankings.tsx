...
        file.name !== '.keep' && 
        new RegExp('\\.(jpg|jpeg|png|webp)$', 'i').test(file.name) // FIX: Prevents babel compile error
      );
...
  	  // FIX: Use right-alignment for this header
...
  	                
  	              const logoDataUrl = canvas.toDataURL('image/jpeg', 0.8);
  	              pdf.addImage(logoDataUrl, 'JPEG', margin, 15, 20, 0); // FIX 1: Was 12, 32, 0
  	              resolve(true);
  	            } catch (error) {
...
  	    const maxScreenshotWidth = availableWidth; // FIX 3: Was * 0.9, now 100% width
  	    const maxScreenshotHeight = 84; // FIX 2: Was 120, now 84 to fit 2 per page
